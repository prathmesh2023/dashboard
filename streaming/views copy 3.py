...
def detect_objects(fr):  # object detection n tracking
    ...
    bbox, label, conf = cv.detect_common_objects(fr, model='yolov4-tiny')
...
def start(request):
    # Start surveillance
    return JsonResponse({"status": "Surveillance started"})

def stop(request):
    # Stop surveillance
    return JsonResponse({"status": "Surveillance stopped"})
